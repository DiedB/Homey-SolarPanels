...
class TrannergyBaseBaseDriver extends Homey.Driver {
    getBaseUrl() {
    ...
    onPair(socket) {
        ...
        socket.on('validate', (data, callback) => {
            TrannergyBase.getToken(this.getBaseUrl(), data.username, data.password)
                .then(token => {
                    systemData = data;
                    currentToken = token;
                    callback(null, true);
        ...
        socket.on('list_devices', (_, callback) => {
            TrannergyBase.getInverters(this.getBaseUrl(), systemData.sid, systemData.username, systemData.password, currentToken)
                .then(devices => {
...
class TrannergyBaseBaseDevice extends Inverter {
    getBaseUrl() {
        throw new Error('Expected override');
    }
            ...
            .then(body => {
                parseXml(body, (error, result) => {
                    if (!error && !result.error) {
                        ...
                        TrannergyBase.getToken(this.getBaseUrl(), settings.username, settings.password)
                            .then(token => {
...
module.exports = {
    Driver: TrannergyBaseBaseDriver,
    Device: TrannergyBaseBaseDevice
};
