...
var base_url = "/solar_api/v1/GetInverterRealtimeData.cgi?Scope=Device&DeviceID=1&DataCollection=CommonInverterData";
var devices = {};
...
function checkProduction(device_data) {
    ...
    http.get(url).then(function (result) {
        if (result.response.statusCode == 200) {
            ...
            var last_output = response.Head.Timestamp;

            if (last_output != device.last_output) {
                device.last_output = last_output;

                var energy = Number(response.Body.Data.DAY_ENERGY.Value / 1000);
                device.energy = energy;
                module.exports.realtime(device_data, "meter_power", energy);

                var power = Number(response.Body.Data.PAC.Value);
                device.power = power;
